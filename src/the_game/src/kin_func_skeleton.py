...
def rotation_3d(omega, theta):
    ...
    skew_omega = skew_3d(omega)

    rot = np.eye(3) + (skew_omega/norm_omega)*np.sin(norm_omega*theta) + (np.linalg.matrix_power(skew_omega, 2)/(norm_omega**2))*(1-np.cos(norm_omega*theta))
...
def homog_2d(xi, theta):
    ...
    z[0] = xi[0]/xi[2]
    z[1] = xi[1]/xi[2]
    p = np.dot(np.dot(np.subtract(np.eye(2), R), rotation_2d(np.pi/2)), z)

    g = np.hstack((R, p))
    g = np.vstack((g, np.zeros((1, 3))))
    g[2,2] = 1
...
def homog_3d(xi, theta):
    ...
    A = np.subtract(np.eye(3), R)
    B = np.dot(skew_3d(omega), v)
    C = np.dot(np.dot(np.dot(omega[:,np.newaxis], omega[np.newaxis,:]), v), theta)
    the_other_thing = np.add(np.dot(A, B), C)/(np.linalg.norm(omega)**2)

    g = np.hstack((R, the_other_thing[:,np.newaxis]))
    g = np.vstack((g, np.zeros((1, 4))))
    g[3,3] = 1
...
def prod_exp(xi, theta):
    ...
    if not theta.shape[0] == xi.shape[1]:
        raise TypeError('The things dont match shape')

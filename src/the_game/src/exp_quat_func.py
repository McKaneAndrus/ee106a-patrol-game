...
def quaternion_to_exp(rot):
    ...
    if theta == 0:
        ...
        omega = np.array(rot[:3])/np.sin(theta/2)
    return (omega, theta)
...
def create_rbt(omega, theta, trans):
    ...
    g = np.vstack((np.hstack((R, trans[:,np.newaxis])), np.array([0,0,0,1])))
    return g
...
def find_v(omega, theta, trans):
    ...
    R = kfs.rotation_3d(omega, theta)
    if (R == np.eye(3)).all():
        v = (trans / np.norm(trans))[:,np.newaxis]
    else:
        omega_vec = omega[:,np.newaxis]
        A = np.dot((np.eye(3) - R), kfs.skew_3d(omega)) + np.dot(omega_vec, omega_vec.T)*theta
        v = np.dot(np.linalg.inv(A), trans)[:,np.newaxis]
    return v
